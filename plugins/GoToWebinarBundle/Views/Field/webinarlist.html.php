...
$webinarlist = unserialize($field['properties']['webinarlist_serialized']);
...
if ( !empty($inForm) && empty($webinarlist)) {
	$errorMessage = $view['translator']->trans('plugin.gotowebinar.field.webinarlist.error');
...
if ($webinarlist) {
	foreach ($webinarlist as $webinarKey => $webinar) {
		$selected = ($webinar === $field['defaultValue']) ? ' selected="selected"' : '';
		$webinar = $view->escape($webinar);
		$options[] = "<option value='$webinarKey' $selected > $webinar </option>";
	}
}
